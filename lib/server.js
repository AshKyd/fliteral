...
function respondError(res, statusCode){
  return respond({
    res,
...
function onRequest(req, res, options){
  ...
  async.waterfall([
    ...
    if(error) return respondError(res, error.statusCode);
...
module.exports = function(options){
  ...
  const server = http.createServer(function(req, res){
    onRequest(req, res, {root});
  });
  server.on('clientError', (err, socket) => socket.end('HTTP/1.1 400 Bad Request\r\n\r\n'));
  server.setTimeout(timeout);
  server.listen(port);
  log.info(`server started on port ${port} with timeout ${timeout}.`);
}
